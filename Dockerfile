...
RUN set -e; \
    pip3 install --default-timeout=100 --no-cache-dir uv; \
    if [ "$USE_CUDA" = "true" ]; then \
    # If you use CUDA the whisper and embedding model will be downloaded on first use
    # fix: pin torch<=2.9.1 - torch 2.10.0 aarch64 wheels cause SIGILL on ARM devices (RPi 4 Cortex-A72) #21349
    pip3 install --default-timeout=100 'torch<=2.9.1' torchvision torchaudio --index-url https://download.pytorch.org/whl/$USE_CUDA_DOCKER_VER --no-cache-dir; \
    HTTP_TIMEOUT=100 uv pip install --system -r requirements.txt --no-cache-dir; \
    ...
    pip3 install --default-timeout=100 'torch<=2.9.1' torchvision torchaudio --index-url https://download.pytorch.org/whl/cpu --no-cache-dir; \
    HTTP_TIMEOUT=100 uv pip install --system -r requirements.txt --no-cache-dir; \
